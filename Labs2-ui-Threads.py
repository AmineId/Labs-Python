...
###
class realTimeThread_retrieve(QtCore.QThread):
    sig_retrieved = QtCore.pyqtSignal(int)
    sig_finished = QtCore.pyqtSignal()

    def __init__(self, parent=None):
        super(realTimeThread_retrieve, self).__init__(parent)
        self.minDate = None
        self.date = None

    def run(self):
        global X_dict, Y_dict
        con = sqlite3.connect(file[0])
        cur = con.cursor()
        mainW.statusBar().showMessage('Real time plotting ... (retrieving coordinates)')
        i = 0
        for id in sorted(ids):
            cur.execute(
                "SELECT latitude FROM TAB WHERE date BETWEEN " + "'{}'".format(self.minDate) + " AND " + "'{}'".format(
                    self.date) + " AND id LIKE " + str("".join([str(s) for s in id[0] if s.isdigit()])))
            tmp = cur.fetchall()
            X_dict[id[0]] = list(itertools.chain(*tmp))
            cur.execute(
                "SELECT longitude FROM TAB WHERE date BETWEEN " + "'{}'".format(self.minDate) + " AND " + "'{}'".format(
                    self.date) + " AND id LIKE " + str("".join([str(s) for s in id[0] if s.isdigit()])))
            tmp = cur.fetchall()
            Y_dict[id[0]] = list(itertools.chain(*tmp))
            frag = int((i / len(ids)) * 100)
            i += 1
            self.sig_retrieved.emit(frag)

        self.sig_finished.emit()


# thread real time plot
class threadToPlot(QtCore.QThread):
    sig_changeMeState = QtCore.pyqtSignal()

    def __init__(self, parent=None):
        super(threadToPlot, self).__init__(parent)

    def run(self):
        frag = sigFetching[0]
        id_ = sigFetching[1]
        pen_ = sigFetching[2]
        symbol_ = sigFetching[3]
        symbSize_ = sigFetching[4]
        brush_ = sigFetching[5]

        # mainW.pBar.show()
        # mainW.pBar.setValue(frag)

        mainW.graphPlot.plot(X_dict[id_[0]], Y_dict[id_[0]], pen=pen_,
                             symbol=symbol_, symbolSize=symbSize_, symbolBrush=brush_)


# thread updating X, Y
class realTimeThread_updating(QtCore.QThread):
    sig_update = QtCore.pyqtSignal(list)
    sig_finished = QtCore.pyqtSignal()

    def __init__(self, parent=None):
        super(realTimeThread_updating, self).__init__(parent)
        self.id = None
        self.pen = None
        self.brush = None
        self.symbol = None
    ...
    def run(self):
        mainW.graphPlot.clear()
        mainW.statusBar().showMessage('Real time plotting ... (plotting targets)')
        i = 0
        for id in sorted(ids):
            ...
            frag = int((i / len(ids)) * 100)
            i += 1
            self.sig_update.emit([frag, self.id, self.pen, self.symbol, self.symbSize, self.brush])
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.threadRetrieveing = None
        self.threadUpdating = None

        self.threadPlotting = threadToPlot()
        self.threadPlotting.finished.connect(self.threadquit)
        ...
        self.pBar.setGeometry(320, 643, 20, 120)
        self.pBar.hide()
    ...
    # Not Handlers
    def progressBarUpdating(self, *args):
        self.pBar.show()
        self.pBar.setValue(args[0])
    ...
    def updatingPlot_pBar(self, *args):
        # fetching from sig and transmitting to the next thread
        global sigFetching
        sigFetching = args[0]
        self.threadPlotting.run()
    ...
    def ReelTimePlot(self):
        ...
        if Enabled_RTP:
            ...
            self.threadRetrieveing = realTimeThread_retrieve()
            self.threadRetrieveing.minDate = minDate
            self.threadRetrieveing.date = date
            self.threadRetrieveing.sig_retrieved.connect(self.progressBarUpdating)
            self.threadRetrieveing.sig_finished.connect(self.threadquit)
            self.threadRetrieveing.start()
            ...
            triggered = False
            self.threadUpdating = realTimeThread_updating()
            self.threadUpdating.sig_update.connect(self.updatingPlot_pBar)
            self.threadUpdating.sig_finished.connect(self.threadquit)
            self.threadUpdating.start()
    ...
    def importCSV(self):
        global df, process, connection, ids, file
        separation = None
        file = QtWidgets.QFileDialog.getOpenFileName(QtWidgets.QFileDialog(),
                                                     'Select dataset',
                                                     os.getcwd(),
                                                     "Comma-separated values (*.csv) ;; SQlite database (*.db *.sdb "
                                                     "*.sqlite *.db3 *.s3db *.sqlite3 *.db2 *.s2db *.sqlite2 *.sl2)")
            ...
            if self.graphPlot.sceneBoundingRect().contains(pos):
                ...
                if process == "csv":
                    ...
                    if Enabled_RTP:
                        ...
                        cursor.execute("SELECT latitude FROM TAB WHERE date BETWEEN " + "'{}'".format(
                            minDate) + " AND " + "'{}'".format(date) +
                        ...
                        cursor.execute(
                            "SELECT id FROM TAB WHERE latitude LIKE " + str(x) + " AND longitude LIKE " + str(y))
                        ...
                        if plt in plotted:
                            ...
                            cursor.execute(
                                "SELECT longitude FROM TAB WHERE id LIKE " + plt + " AND latitude LIKE " + str(x[0][0]))
                            ...
                            if len(y) > 0 and len(x) > 0:
                                ...
                                self.statusBar().showMessage(
                                    'x:' + str(x) + ', y:' + str(y) + ', ID: mobile ' + str(plt))
